    ...
    def __load_main_process_info(config_file: pd.ExcelFile) -> Dict[str, ScenarioConfig]:
        ...
        for row in [AsciiFilter(row) for _, row in main_collect_sheet.iterrows()]:
            ...
            if 'loop_dns' in row.ITEM:
                ...
                ###### SHOW/DOWN/UPのいずれかを検知したら"mode_check_result"をTrueに、それ以外はFalse
                mode_check_result = True
                if mode_check_result:
                    ## サブシート名のパタン抽出
                    ## 文字列の初めから、A~Z中3文字、0~9までの数字3文字を抽出 
                    pattern = re.compile(r'[A-Z]{3}[0-9]{3}')
                    # 'loop_dns'が含まれる行の"COMMAND"列から上記パタン抽出（抽出結果はSUB001/SUB002/SUB003）
                    sub_sheet_name: str = pattern.search(row.COMMAND).group(0)
                    # 抽出したサブシート名のエクセルシートを読み込む
                    sub_collect_sheet = config_file.parse(sub_sheet_name)
                    # 抽出したサブシート名と読み込んだエクセルファイルを引数にサブ設定情報ロードを呼び出す
            ...
            if not pd.isnull(row.NODE):
                ...
                # 読み込み中のMAINシート行の"COMMAND"列に'SUB'が含まれる、かつサブシート読み込みフラグがTrueの場合
                if 'SUB' in row.COMMAND and sub_sheet_flg :
                    # 実行コマンド設定情報辞書にサブシート名をキーとして追加する
...
if __name__ == '__main__':
    ...
    print(config.mainConfigs['main']._ScenarioConfig__commandConfigs['MAIN_start'])
    # print()
    # print()
    # print(config._Config__connectConfigs)
    # for key, val in config.mainConfigs['main']._ScenarioConfig__commandConfigs.items():
    #     print(key)
    #     if key == 'SUB001':
    #         for key, val in val['amf_dns_show']._ScenarioConfig__commandConfigs.items():
    #             print(key)
    #             print(val)
    #             print()
    #             print()
    #     elif key == 'SUB002':
    #         for key, val in val['amf_dns_del']._ScenarioConfig__commandConfigs.items():
    #             print(key)
    #             print(val)
    #             print()
    #             print()
    #     elif key == 'SUB003':
    #         for key, val in val['amf_dns_add']._ScenarioConfig__commandConfigs.items():
    #             print(key)
    #             print(val)
    #             print()
    #             print()
